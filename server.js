...
// ---------------------------------------------------------------------------
// UPDATED: breakdownStory() — only the systemPrompt content and the addition
// of a small conditional "extraGuidance" string have changed. The function
// signature, the JSON schema it returns (image_prompt / narration / movement),
// the API endpoint, and the number of API calls (still exactly one) are all
// unchanged, so nothing downstream needs to change.
// ---------------------------------------------------------------------------
async function breakdownStory(story) {
  const systemPrompt = `You are a film storyboard assistant and a careful, research-grounded visual director.

Step 1 (internal, do not output): From the story, identify the specific historical subject, location/region, time period, and the concrete objects, activities or evidence the story actually describes. Use this to ground every scene.

Step 2: Break the story into 6-10 concise cinematic scenes for a short narrated video.
...
- Stay tied to the specific subject, region and time period identified in Step 1. Never drift into generic or unrelated imagery.
- Prefer the specific objects, environments and activities mentioned in the narration over generic stand-ins (e.g. "terracotta fragments in reddish soil" instead of "African village").
- Do not invent specific famous monuments, artifacts, clothing or events the story doesn't support.
- No glowing maps, floating light lines, skulls, fantasy ruins, random statues, or sci-fi/supernatural imagery, unless the narration explicitly describes them.
- Depict historical/archaeological objects as a realistic documentary photograph or cinematic historical reconstruction — never claim it's an authentic photo of a specific real find.
- For uncertain details, use a plausible atmospheric reconstruction rather than inventing specifics.
- Keep character appearance, setting and visual style consistent across scenes so they read as one continuous film, not disconnected stock photos.
- Match the actual region/culture of the story (e.g. Nigerian landscapes and architecture for a Nigerian story), not a different or ambiguous one.

Return valid JSON only, nothing else.`;

  const lowerStory = story.toLowerCase();
  const extraGuidance = lowerStory.includes('nok')
    ? `\n\nThis story is about the Nok culture of central Nigeria. Prioritize: Nok-style terracotta sculpture, archaeological excavation sites, ancient farming communities, ironworking reconstruction, pottery fragments, soil/stratigraphy layers, researchers studying evidence, and Nigerian savanna/central-Nigeria landscapes. Do not present generic African imagery as if it specifically represents Nok.`
    : '';
  ...
  const res = await fetch('https://gen.pollinations.ai/v1/chat/completions', {
    ...
    body: JSON.stringify({
      model: 'openai',
      messages: [
        { role: 'system', content: systemPrompt + extraGuidance },
...
// ---------------------------------------------------------------------------
// END of updated section. Everything below is unchanged from the original.
// ---------------------------------------------------------------------------
